...
bool Aluno::operator> (const Aluno & other) const {
	//Comparamos o nome de ambos e caso seja igual usamos matricula para desempate.
    if (nome < other.nome) 
		return true;
    if (nome == other.nome && matricula < other.matricula) 
		return true;
    return false;
}

bool Aluno::operator< (const Aluno & other) const {
	//comparamos o nome de ambos e caso seja lexicograficamente igual usamos a matricula como desempate
    if (nome > other.nome) 
		return true;
    if (nome == other.nome && matricula > other.matricula)
	 	return true;
