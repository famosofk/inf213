...
MySetIterator<T>  MySetIterator<T>::operator++() {
	//Como funciona o incremento? Basicamente vamos um nodo para a direita e depois vamos todos os nodos possíveis a esquerda, com o objetivo de ir para o menor nodo maior que o atual.
	//Caso não haja nodos a direita, por exemplo, estamos em uma "ponta", o que fazemos é olhar para pai e vermos se estamos a direita dele, fazemos isso até não estarmos mais.
...
MySetIterator<T>  MySetIterator<T>::operator--() {
	...
	//Primeiro vamos o primeiro a esquerda e depois vamos o máximo a direita pra garantir que seja o menor maior. No else, vemos se estamos sempre a esquerda do pai(), fazemos isso até não estar mais.
	//Em seguida, voltamos ao parent, pois o parent é menor que o filho caso o filho esteja a direita
	if(ptr->left) { 
		ptr = ptr->left;
		while(ptr->right) 
			ptr=ptr->right;
