...
void MyHashMap<KEY, VALUE, HASH>::reHash(int newSize){
	...
	MyVec<MyList2<std::pair<KEY,VALUE> > > vecAux = table;

	MyVec<MyList2<std::pair<KEY,VALUE>>> a;
	table = a;
	table.resize(newSize);
	numElems = 0;

	typename MyVec<MyList2<std::pair<KEY,VALUE>>>::iterator it = vecAux.begin();

	for(int i = 0; it != vecAux.end() ;it++, i++){
		typename MyList2<std::pair<KEY, VALUE>>::iterator itera2 = (*it).begin();
		for(int j = 0; itera2 != (*it).end() ; j++, itera2++){
			this->set((*itera2).first, (*itera2).second);
		}
	}
}
...
void MyHashMap<KEY,VALUE,HASH>::set(const  KEY &k, const VALUE &v) {
	...
	int position = funcaoHash(k)%tableSize;
	MyList2<std::pair<KEY, VALUE>> list = table[position];
	typename MyList2<std::pair<KEY, VALUE>>::iterator it = list.begin();
	bool hasKey = false;
	...
	if(!hasKey) {list.push_back(make_pair(k, v)); this->numElems++;}
	table[position] = list;
